...
@section('meta-title')
  ...
  <meta name="author" content="{{ $data->created_by }}">
  <meta name="description" content="{{ substr($data->content, 0, 100) }}">
@endsection

@section('content')
  <main class="fh-page pt-5 text-dark bg-light">
    {{-- title page  --}}
    <section class="page-header bg-tertiary">
        <div class="container">
            <div class="row">
                <div class="col-8 mx-auto text-center">
                    <h2 class="mb-3 text-capitalize fw-bold">{{ $data->title }}</h2>
                    <ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
                        <li class="list-inline-item"><a href="{{ route('home') }}">Home</a>
                        </li>
                        <li class="list-inline-item">/ &nbsp; <a href="{{ route('artikel') }}">Artikel</a>
                        </li>
                        <li class="list-inline-item">/ &nbsp; <a href="{{ route('artikel.detail', $data->slug) }}">{{ $data->title }}</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    {{-- title page  --}}
    <section class="w-100 bg-body-secondary py-5">
      <div class="w-100 container">
        <img src='{{ url($data->image) }}' style="width: 100%;height: 350px;object-fit: cover" alt="{{ $data->slug }}">
        <div class="mt-3"> 
          <h1>{{ $data->title }}</h1>
          <div class="d-flex color-ac gap-3 mt-2">
              <div class="gap-2 d-flex box-supporter">
                  <i class="bi-person-fill mb-0"></i>
                  <span class="mb-0">{{ $data->created_by }}</span>
              </div>
              <div class="gap-2 d-flex box-supporter">
          ...
          <hr>
          <div class="content-artikel w-100 overflow-x-hidden">
            <div>
              {!! $data->content !!}
            </div>
          </div>
          ...
          <div class="d-flex gap-3">
            <button class="btn btn-primary" onclick="share()" id="sharetext"><i class="bi-share-fill mx-2"></i>Share</button>
            <a class="btn btn-primary" href="https://wa.me/?text={{ urlencode('Ayo cek artikel terbaru Indramedia! Cek disini ' . url()->current()) }}">
              <i class="bi-whatsapp"></i>
            </a>
            <a title="facebook" target="popup" href="http://www.facebook.com/sharer/sharer.php?u={{ url()->current() }}','Facebook Share','width=600,height=600')"class="btn btn-primary">
  ...
  <script>
    function share() {
      ...
      const res = window.navigator.clipboard.writeText(window.location.href);
      if (res) {
        $('#sharetext').text('Link Disalin !')
        setTimeout(() => {
          $('#sharetext').html('<i class="bi-share-fill mx-2"></i>Share')
        }, 1500);
      }
    }
  </script>
