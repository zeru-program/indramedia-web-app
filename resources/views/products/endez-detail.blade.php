        ...
        <section class="align-items-center d-flex flex-column pt-5">
            <div class="container d-flex flex-column">
                <div class="w-100 d-flex justify-content-start gap-2 contain-head-detail-product">
                    <div class="rounded-2 mb-3 contain-img-produk-detail img-thumbnail" style="height: 400px">
                ...
                <div class="w-100 row mt-3">
                    <div class="col-6">
                        ...
                        <button class="btn bg-sc w-100 color-ac" data-bs-toggle="modal" data-bs-target="#modalAddCart"><i
                                class="bi-cart-plus-fill mx-2"></i>Tambahkan Keranjang</button>
                    </div>
                    <div class="col-6">
                        <button class="btn btn-primary w-100"
                            onclick="buyNow('{{ $data->is_promo == 1 ? $data->promo->promo_price : $data->price }}')">Beli
                            Sekarang</button>
                    </div>
                </div>
                <div class="mt-5 d-none" id="containForm">
                    <h3 class="fw-bold">Formulir Pemesanan</h3>
                    <form action="{{ route('produk.endez.create') }}" enctype='multipart/form-data' method="POST"
                        class="mt-2" id="formOrder">
                        @csrf
                        <div class="row col-md-12">
                            <div class="mb-2 col-md-12">
                                <label for="order_produk">Nama produk</label>
                                <input type="text" name="order_product" value="{{ $data->name }}" required disabled
                                    class="form-control" id="formOrder" class="mt-2">
                                ...
                                @if (
                                    ...
                                    <input type="text" name="order_price" hidden
                                        value="{{ floatval($data->promo->promo_price) }}">
                                @else
                            ...
                            <div class="row mb-2 col-md-12">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" name="order_phone" placeholder="masukan nomor handphone (wa) mu.."
                                        ...
                                        id="">
                                    <input type="text" name="order_phone" hidden required
                        ...
                        <div class="row col-md-12">
                            <input type="number" name="order_is_onefile" required hidden
                                value="{{ $data->is_onefile }}">
                            {{-- @dd($data->is_multiplefile) --}}
                            <input type="number" name="order_is_multiplefile" required hidden
                                value="{{ $data->is_multiplefile }}">
                            @if ($data->is_multiplefile == 1)
                                <div class="mb-2 col-md-6">
                                    <label for="order_qty">Jumlah pesanan</label>
                                    <input type="number" name="order_qty" min="1" max="{{ $data->stock }}"
                                        placeholder="Masukan jumlah pesanan" required
                                        oninput="handleQtyChange(event, '{{ $data->is_promo == 1 ? $data->promo->promo_price : $data->price }}', true)"
                            ...
                            @elseif ($data->is_onefile == 1)
                                <div class="mb-2 col-md-6">
                                    ...
                                    <input type="number" name="order_qty" min="1" max="{{ $data->stock }}"
                                        placeholder="Masukan jumlah pesanan" required
                                        oninput="handleQtyChange(event, '{{ $data->is_promo == 1 ? $data->promo->promo_price : $data->price }}', false)"
                                        min="1" class="form-control">
                                </div>
                            @else
                                <div class="mb-2 col-md-6">
                                    <label for="order_qty">Jumlah pesanan</label>
                                    <input type="number" name="order_qty" min="1" max="{{ $data->stock }}"
                                        placeholder="Masukan jumlah pesanan" required id="qty-input"
                                        oninput="handleQtyChange(event, '{{ $data->is_promo == 1 ? $data->promo->promo_price : $data->price }}', false)"
                                        class="form-control">
                                </div>
                            @endif
                            ...
                            <div id="appendInputFile">
                                <div class="row col-md-12">
                                    <div class="mb-2 col-md-12">
                                        ...
                                        <input type="file" name="order_files1" disabled class="form-control"
                                            style="height: auto !important;padding-bottom: 6px !important" id="">
                                    </div>
                                </div>
                            </div>
                        @elseif ($data->is_onefile == 1)
                            <div id="appendInputFile">
                                <div class="row col-md-12">
                                    <div class="mb-2 col-md-12">
                                        <label for="order_files">File produk </label>
                                        <input type="file" name="order_onefile" class="form-control"
                        ...
                        <div class="row col-md-12">
                            <div class="mt-2 col-md-12">
                                <button class="btn bg-success w-100 color-pm" type="button" onclick="handleCheckout()">
                                    <i class="bi-cart"></i>
...
@section('scripts')
    ...
    <script>
        function handleQtyChange(e, price, isFile) {
            ...
            $('#pajak-admin').text(new Intl.NumberFormat('id-ID', {
                style: 'currency',
                currency: 'IDR'
            }).format(pajak.toFixed(2)));
            $('#displayTotal').text(
